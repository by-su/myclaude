...
description: >
  ...
  review 모드(기본값, 기획~와이어프레임 단계별 승인)와 auto 모드(완전 자동, 사용자가 명시적으로 요청한 경우에만)를 지원한다.
...
| 모드 | 설명 |
|------|------|
| **review** (기본값) | Stage 1·2·3·4·4.5·5 완료 후 사용자 승인을 받고 다음 단계 진행. Stage 5.5·6(빌드)부터는 자동 |
...
```
review 모드로 진행합니다.
Stage 1(리서치) → Stage 2(아이디어) → Stage 3(시장 검증) → Stage 4(PRD) → Stage 4.5(팔레트) → Stage 5(와이어프레임)
각 단계 완료 후 산출물 요약을 보여드리고 승인을 요청합니다. Stage 6(빌드)부터는 자동입니다.
```
...
review 모드에서는 **Stage 1, 2, 3, 4, 4.5, 5 완료 후** 사용자에게 산출물 요약을 보여주고 승인을 받는다. 승인 게이트는 `AskUserQuestion` 도구로 진행한다.

**승인 게이트 동작:**

```
1. Stage 완료 후, 핵심 산출물을 읽어 요약을 사용자에게 출력
2. AskUserQuestion으로 묻는다: "계속 진행할까요?"
3. 선택지:
   - "진행" → 다음 Stage로
   - "수정 요청" → 사용자 피드백을 받아 해당 Stage 서브에이전트 재호출
...
**수정 요청 처리:**
- 사용자가 "아이디어 2번은 빼줘", "MVP 범위 좁혀줘" 같은 피드백을 주면, 해당 피드백을 컨텍스트에 추가하여 그 Stage 서브에이전트를 재호출한다
- 재호출 시 이전 산출물을 덮어쓴다
...
- 한 번에 너무 많은 Stage를 묶어 진행하지 말고 게이트 단위로 끊는다. review 모드의 핵심은 사용자 개입 지점이다.
