...
| `cto-architect` | PRD(파일 경로 또는 본문) → 실용적 시니어 CTO 관점으로 8단계 분석(기능 인벤토리 → 도메인 경계 → 시스템 컴포넌트 → API 설계 → 데이터 ERD → 비기능 요구사항 → 리스크·트레이드오프 → HTML 생성). Mermaid 기반 시스템 컴포넌트 다이어그램 + ERD가 포함된 자기완결 HTML(`architecture/{prd-basename}.html`). 모든 추천은 패턴 수준(RDB/메시지 브로커/오브젝트 스토리지)이며 MVP 오버엔지니어링(마이크로서비스·CQRS·이벤트 소싱) 거부. `wireframe-architect`(화면 구조)와 짝꿍. |
