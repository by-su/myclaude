...
│   ├── code-reviewer.md
│   └── code-reviewer/rules/   # 스택별 리뷰 룰 (react-nextjs, typescript, ...)
├── agent-memory/           # 에이전트 영속 메모리 (→ ~/.claude/agent-memory 심볼릭 링크)
├── hooks/                  # Claude Code hook 스크립트 (→ ~/.claude/hooks 심볼릭 링크)
│   └── auto-code-review.sh    # Stop hook: 파일 수정 턴 끝에 code-reviewer 자동 호출
├── setup/
│   └── init/
│       └── install.sh      # skills/agents/hooks 심볼릭 링크 + settings.json 패치 + MCP 설정
